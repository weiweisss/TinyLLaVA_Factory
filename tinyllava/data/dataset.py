...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if isinstance(data_args.image_processor, dict):
            self.image_preprocessors = {
                key: ImagePreprocess(processor, data_args)
                for key, processor in data_args.image_processor.items()
            }
            self.use_multi_processors = True
        else:
            self.image_preprocess = ImagePreprocess(data_args.image_processor, data_args)
            self.use_multi_processors = False
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            # Process image with appropriate processor(s)
            if self.use_multi_processors:
                processed_images = {}
                for key, processor in self.image_preprocessors.items():
            ...
            # print(f'{i}:{sources}')
            if self.use_multi_processors:
                # Create dummy images for each processor
                dummy_images = {}
                for key, processor in self.image_preprocessors.items():
                    crop_size = getattr(processor.image_processor, 'crop_size',
                                        getattr(processor.image_processor, 'size'))
                    dummy_images[key] = torch.zeros(3, crop_size['height'], crop_size['width'])
                data_dict['image'] = dummy_images
            else:
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            # Check if we're using multiple processors
            if isinstance(images[0], dict):
                # Handle multiple processors case
                batch['images'] = {}
                for key in images[0].keys():
                    key_images = [img[key] for img in images]
                    if all(x is not None and x.shape == key_images[0].shape for x in key_images):
                        batch['images'][key] = torch.stack(key_images)
                    else:
                        batch['images'][key] = key_images
            else:
